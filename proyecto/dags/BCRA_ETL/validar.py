...
def validar_tc_minorista(**kwargs):
    ...
    ti = kwargs['ti']
    df_json = ti.xcom_pull(key='TipoCambioMinorista', task_ids='CargaTipoCambioMinorista')
    df = pd.read_json(StringIO(df_json))
...
def validar_tc_mayorista(**kwargs):
    ...
    ti = kwargs['ti']
    df_json = ti.xcom_pull(key='TipoCambioMayorista', task_ids='CargaTipoCambioMayorista')
    df = pd.read_json(StringIO(df_json))
